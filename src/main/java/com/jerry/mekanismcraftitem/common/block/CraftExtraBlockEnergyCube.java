package com.jerry.mekanismcraftitem.common.block;
...
public class CraftExtraBlockEnergyCube extends BlockTile.BlockTileModel<ExtraTileEntityEnergyCube, CraftMachine<ExtraTileEntityEnergyCube>> {
    ...
    public CraftExtraBlockEnergyCube(CraftMachine<ExtraTileEntityEnergyCube> type) {
        super(type, Properties.of().strength(2.0F, 2.4F).requiresCorrectToolForDrops().dynamicShape().mapColor(MapColor.DEEPSLATE));
    }
    ...
    public @NotNull VoxelShape getShape(@NotNull BlockState state, @NotNull BlockGetter world, @NotNull BlockPos pos, @NotNull CollisionContext context) {
        ExtraTileEntityEnergyCube energyCube = (ExtraTileEntityEnergyCube) WorldUtils.getTileEntity(ExtraTileEntityEnergyCube.class, world, pos, true);
        int index;
        if (energyCube == null) {
            ...
            if (energyConfig == null) {
                ...
                Direction facing = ((AttributeStateFacing) Attribute.get(this, AttributeStateFacing.class)).getDirection(state);
                index = getIndex(isSideEnabled(energyConfig, facing, Direction.UP), isSideEnabled(energyConfig, facing, Direction.DOWN), isSideEnabled(energyConfig, facing, Direction.SOUTH), isSideEnabled(energyConfig, facing, Direction.NORTH), isSideEnabled(energyConfig, facing, Direction.EAST), isSideEnabled(energyConfig, facing, Direction.WEST), facing == Direction.EAST || facing == Direction.WEST, facing == Direction.DOWN || facing == Direction.UP);
            }
        }
